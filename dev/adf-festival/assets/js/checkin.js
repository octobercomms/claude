/**
 * ADF Festival — door check-in PWA.
 *
...
(function () {
    ...
    var cfg = window.ADF_CHECKIN || {};
    var root = document.getElementById('adf-checkin');
    if (!root) { return; }
    ...
    function go(step) {
        root.querySelectorAll('.adf-ci-step').forEach(function (s) {
    ...
    get('/checkin-events', {}).then(function (events) {
        var box = document.getElementById('adf-ci-events');
        box.innerHTML = (events || []).map(function (e) {
            return '<button class="adf-btn adf-ci-event" data-id="' + e.id + '">' + esc(e.title) + '</button>';
        }).join('') || '<p>No events with tickets.</p>';
        box.querySelectorAll('.adf-ci-event').forEach(function (btn) {
            btn.addEventListener('click', function () { state.eventId = parseInt(btn.dataset.id, 10); go('pin'); });
        });
    });

    /* ---- PIN ---- */
    document.getElementById('adf-ci-pin-go').addEventListener('click', function () {
        var pin = document.getElementById('adf-ci-pin').value.trim();
        var msg = document.getElementById('adf-ci-pin-msg');
    ...
    function renderVenues(venues) {
        var box = document.getElementById('adf-ci-venues');
        var list = (venues && venues.length) ? venues : ['Main door'];
        box.innerHTML = list.map(function (v) {
            return '<button class="adf-btn adf-ci-venue" data-v="' + esc(v) + '">' + esc(v) + '</button>';
        }).join('');
        box.querySelectorAll('.adf-ci-venue').forEach(function (btn) {
            btn.addEventListener('click', function () {
                state.venue = btn.dataset.v;
                document.getElementById('adf-ci-venue-name').textContent = state.venue;
    ...
    function startScanner() {
        ...
        scanner = new window.Html5Qrcode('adf-ci-reader');
        scanning = true;
    ...
    document.getElementById('adf-ci-manual-go').addEventListener('click', function () {
        var t = document.getElementById('adf-ci-manual').value.trim();
        if (t) { submit(t); document.getElementById('adf-ci-manual').value = ''; }
    });
    ...
    function submit(token) {
        post('/checkin-scan', { token: token, event_id: state.eventId, pin: state.pin, venue: state.venue })
            .then(function (res) {
                var s = res.body || {};
                overlay(s.status, s);
                if (s.status === 'valid') { state.count++; document.getElementById('adf-ci-count').textContent = state.count; }
                refreshStats();
            });
    }

    function overlay(status, info) {
        var o = document.getElementById('adf-ci-overlay');
        var inner = o.querySelector('.adf-ci-overlay-inner');
        var map = {
        ...
        o.className = 'adf-ci-overlay adf-ci-' + m[0];
        inner.innerHTML = '<div class="adf-ci-big">' + esc(m[1]) + '</div><div>' + esc(m[2]) + '</div>';
        o.hidden = false;
    ...
    function refreshStats() {
        get('/checkin-stats', { event_id: state.eventId, pin: state.pin }).then(function (res) {
            if (res.error) { return; }
            var box = document.getElementById('adf-ci-stats');
    ...
    if (!window.Html5Qrcode) {
        var reader = document.getElementById('adf-ci-reader');
