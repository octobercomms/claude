...
class HGDF_Spam {
	...
	public static function turnstile_site_key() {
		return trim( (string) get_option( 'hgd_form_turnstile_site_key', '' ) );
	}

	public static function turnstile_secret_key() {
		return trim( (string) get_option( 'hgd_form_turnstile_secret_key', '' ) );
	}
	...
	public static function verify_turnstile( $token ) {
		...
		$res = wp_remote_post( 'https://challenges.cloudflare.com/turnstile/v0/siteverify', array(
			...
			'body'    => array(
				...
				'response' => $token,
				'remoteip' => HGDF_Submission::client_ip(),
			),
		) );
		if ( is_wp_error( $res ) ) {
	...
	public static function rate_limit_ok( $form_id, $max_per_window = 5, $window_seconds = 600 ) {
		$ip = HGDF_Submission::client_ip();
		if ( ! $ip ) {
			return true;
		}
		$key = 'hgd_form_rl_' . md5( $form_id . '|' . $ip );
	...
	public static function honeypot_ok( $request_data ) {
		...
		return empty( $request_data['hgd_form_hp'] ) && empty( $request_data['hgd_form_hp_email'] );
	}
}
