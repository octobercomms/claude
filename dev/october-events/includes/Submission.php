 ...
 * `oe_*` CPTs because it only ever touches shared `_oe_` meta.
 */
final class Submission {
    ...
    public static function create(string $type, array $data, int $account_id, string $tier = Fields::TIER_FREE) {
        ...
        if ($slug === '' || ! in_array($type, PostTypes::listing_types(), true)) {
            return new \WP_Error('oe_invalid_type', __('Unknown listing type.', 'october-events'));
        }
        if (! $account_id) {
            return new \WP_Error('oe_no_account', __('A valid account is required.', 'october-events'));
        }

        $wp_user = (int) get_post_meta($account_id, '_oe_wp_user_id', true) ?: get_current_user_id();

        // Persist as a draft first so we never lose data if payment fails.
        $post_id = wp_insert_post([
            'post_type'    => $slug,
            'post_status'  => 'draft',
            'post_title'   => sanitize_text_field((string) ($data['title'] ?? __('Untitled submission', 'october-events'))),
        ...
        foreach ((array) ($data['meta'] ?? []) as $key => $value) {
            update_post_meta($post_id, '_oe_' . sanitize_key($key), self::sanitize_meta($value));
        }
        ...
        if ($amount > 0) {
            if (! StripeConnector::is_ready()) {
                ...
                return new \WP_Error('oe_stripe_unconfigured', __('Payments are not available right now.', 'october-events'));
            }
            ...
            if (($intent['id'] ?? '') === '') {
                Fields::set($post_id, 'status', Fields::STATUS_PENDING_PAYMENT);
                return new \WP_Error('oe_payment_failed', __('Could not start payment.', 'october-events'));
            }
        ...
        if (! empty($overrides[$type])) {
            ...
            if ($refund_amount !== '') {
                $template .= "\n\n" . sprintf(
                    ...
                    __('A full refund of %s has been processed to your original payment method and should appear within 3–5 business days.', 'october-events'),
                    '{refund_amount}'
                );
            }
        }
