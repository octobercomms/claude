  ...
  return (
    ...
    >
      <div className="card" style={{ marginBottom: 'var(--s5)' }}>
        ...
        {err && <div className="callout callout-danger" style={{ marginBottom: 'var(--s3)' }}>{err}</div>}
        {!prospects.length ? (
          <div style={{ color: 'var(--text-subtle)', fontSize: 'var(--fs-body)', padding: 'var(--s3) 0' }}>
            {dfsUnlocked
          ...
          <div style={{ padding: 0 }}>
            <table className="table">
              <thead>
                <tr>
                  <th className="caption" style={{ padding: 'var(--s2) var(--s3)' }}>Source</th>
                  <th className="caption" style={{ padding: 'var(--s2) var(--s3)' }}>Linked to</th>
                  <th className="caption" style={{ padding: 'var(--s2) var(--s3)', textAlign: 'right' }}>DR</th>
                  <th className="caption" style={{ padding: 'var(--s2) var(--s3)', textAlign: 'right' }}>Score</th>
              ...
              <tbody>
                {prospects.slice(0, 50).map(p => (
                  <tr key={p.id} style={{ borderBottom: '1px solid #f5f5f5' }}>
                    ...
                    <td style={{ padding: 'var(--s2) var(--s3)', fontSize: 'var(--fs-caption)', color: 'var(--text-subtle)' }}>{p.competitor_domain || '—'}</td>
                    <td style={{ padding: 'var(--s2) var(--s3)', fontSize: 'var(--fs-caption)', textAlign: 'right' }}>{p.domain_rank ?? '—'}</td>
                    <td style={{ padding: 'var(--s2) var(--s3)', fontSize: 'var(--fs-caption)', textAlign: 'right', fontWeight: 700 }}>{p.relevance_score}</td>
                    <td style={{ padding: 'var(--s2) var(--s3)', fontSize: 'var(--fs-caption)', textTransform: 'uppercase', letterSpacing: 0.5, color: 'var(--text-muted)' }}>{p.status}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="card" style={{ marginBottom: 'var(--s5)' }}>
        ...
        {!!responses.length && (
          <div style={{ display: 'grid', gridTemplateColumns: '240px 1fr', gap: 'var(--s5)' }}>
            <div>
              {responses.map(r => (
                <div key={r.id} className="card"
                  ...
                  <div style={{ fontSize: 'var(--fs-caption)', color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 'var(--s1)' }}>
                  ...
                  {r.deadline && (
                    <div style={{ fontSize: 'var(--fs-caption)', color: new Date(r.deadline) < new Date() ? 'var(--negative)' : 'var(--text-subtle)', marginTop: 'var(--s1)' }}>
            ...
            <div>
              {activeResponse ? (
                ...
                <div style={{ color: 'var(--text-subtle)', padding: 'var(--s5)' }}>Pick a response on the left, or click "+ New response" to draft a new one.</div>
              )}
            </div>
          </div>
        )}
          ...
          <div key={t.key} className="card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 'var(--s2)' }}>
              ...
              <span style={{ fontSize: 'var(--fs-caption)', fontWeight: 700, letterSpacing: 0.5, textTransform: 'uppercase',
                              padding: 'var(--s1) var(--s2)', borderRadius: 'var(--r-sm)',
