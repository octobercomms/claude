 ...
 * max-uses). Backed by the adf_promo_codes table.
 */
final class Promo {
    ...
    public static function validate(string $code, int $event_id, float $subtotal) {
        ...
        if (! $promo) {
            return new \WP_Error('adf_promo_invalid', __('That code is not valid.', 'adf-festival'));
        }
        if (! $promo->active) {
            return new \WP_Error('adf_promo_inactive', __('That code is no longer active.', 'adf-festival'));
        }
        if ($promo->event_id !== null && (int) $promo->event_id !== $event_id) {
            return new \WP_Error('adf_promo_event', __('That code does not apply to this event.', 'adf-festival'));
        }
        if ($promo->expires_at !== null && strtotime((string) $promo->expires_at) < current_time('timestamp', true)) {
            return new \WP_Error('adf_promo_expired', __('That code has expired.', 'adf-festival'));
        }
        if ($promo->max_uses !== null && (int) $promo->used_count >= (int) $promo->max_uses) {
            return new \WP_Error('adf_promo_used', __('That code has reached its usage limit.', 'adf-festival'));
        }
