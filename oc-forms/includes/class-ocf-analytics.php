...
/**
 * Analytics + tracking for nvelope Forms.
 *
 * Pipeline of a single visitor:
 *   1. Page renders → JS POSTs to /view → row in wp_ocf_views.
 *   2. User starts filling → JS POSTs to /start → row in wp_ocf_submissions.
...
class OCF_Analytics {
	...
	public static function record_view( $form_id, $session_hash ) {
		...
		$wpdb->insert( self::views_table(), array(
			...
			'ip_address'   => OCF_Submission::client_ip(),
	...
	public static function update_progress( $submission_id, $step_reached, $seconds_active ) {
		...
		$existing = $wpdb->get_row( $wpdb->prepare(
			'SELECT step_reached, seconds_active FROM ' . OCF_Submission::table() . ' WHERE id = %d',
			$submission_id
		), ARRAY_A );
		if ( ! $existing ) { return; }
		$wpdb->update( OCF_Submission::table(), array(
	...
	public static function form_stats( $form_id, $from = '', $to = '' ) {
		...
		$starts   = (int) $wpdb->get_var( 'SELECT COUNT(*) FROM ' . OCF_Submission::table() . " WHERE {$where_s}" );
		$partials = (int) $wpdb->get_var( 'SELECT COUNT(*) FROM ' . OCF_Submission::table() . " WHERE {$where_s} AND status = 'partial'" );
		$completes= (int) $wpdb->get_var( 'SELECT COUNT(*) FROM ' . OCF_Submission::table() . " WHERE {$where_s} AND status = 'complete'" );

		// Time-on-form: seconds_active for completed submissions only (clean signal).
		$times = $wpdb->get_col( 'SELECT seconds_active FROM ' . OCF_Submission::table() . " WHERE {$where_s} AND status = 'complete' AND seconds_active > 0 ORDER BY seconds_active ASC" );
	...
	public static function funnel( $form_id, $from = '', $to = '' ) {
		...
		foreach ( $steps as $idx => $step ) {
			$reached = (int) $wpdb->get_var(
				'SELECT COUNT(*) FROM ' . OCF_Submission::table() . " WHERE {$where} AND step_reached >= " . (int) $idx
			);
			$out[] = array(
				'step_index' => $idx,
		...
		$completes = (int) $wpdb->get_var( 'SELECT COUNT(*) FROM ' . OCF_Submission::table() . " WHERE {$where} AND status = 'complete'" );
		$out[] = array(
	...
	public static function timeseries( $form_id, $from = '', $to = '' ) {
		...
		$starts = $wpdb->get_results( $wpdb->prepare(
			'SELECT DATE(created_at) AS d, COUNT(*) AS c FROM ' . OCF_Submission::table() . ' WHERE form_id = %d AND created_at BETWEEN %s AND %s GROUP BY DATE(created_at)',
			$form_id, $from_ts, $to_ts
		), ARRAY_A );
		$completes = $wpdb->get_results( $wpdb->prepare(
			"SELECT DATE(completed_at) AS d, COUNT(*) AS c FROM " . OCF_Submission::table() . " WHERE form_id = %d AND status = 'complete' AND completed_at BETWEEN %s AND %s GROUP BY DATE(completed_at)",
			$form_id, $from_ts, $to_ts
		), ARRAY_A );
	...
	public static function visitor_session() {
		if ( ! empty( $_COOKIE['ocf_sess'] ) && preg_match( '/^[a-zA-Z0-9_-]{16,64}$/', $_COOKIE['ocf_sess'] ) ) {
			return $_COOKIE['ocf_sess'];
		}
