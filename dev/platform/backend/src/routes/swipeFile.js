...
const swipeProcessor = require('../services/swipeProcessor');
...
router.post('/clients/:clientId/swipe', async (req, res) => {
  try {
    const item = await swipe.create(req.params.clientId, req.body || {}, req.user?.id);
    swipeProcessor.kick(); // process now if the platform handles transcription (no worker)
    res.status(201).json(item);
  } catch (err) { res.status(err.status || 500).json({ error: err.message }); }
});
router.post('/clients/:clientId/swipe/:id/retry', async (req, res) => {
  try { const it = await swipe.retry(req.params.clientId, id(req)); swipeProcessor.kick(); res.json(it); }
  catch (err) { res.status(err.status || 500).json({ error: err.message }); }
});
