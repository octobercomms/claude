...
(function ($) {
	...
	function loadProducts(page = 1) {
		...
		showStatus(wbe.i18n.loading, 'info');
	...
	$saveBtn.on('click', function () {
		...
		if (!changeList.length) {
			showStatus(wbe.i18n.noChanges, 'info');
			return;
		}

		$saveBtn.prop('disabled', true).text(wbe.i18n.saving);
		showStatus(wbe.i18n.saving, 'info');

		$.post(wbe.ajaxUrl, {
			action:  'wbe_save_changes',
			nonce:   wbe.nonce,
			...
			if (!response.success) {
				const msg = response.data?.errors?.join(' | ') || wbe.i18n.saveError;
			...
			showStatus(wbe.i18n.saved, 'success');
			updateToolbar();
		}).fail(function () {
			$saveBtn.prop('disabled', false).text('Save All Changes');
			showStatus(wbe.i18n.saveError, 'error');
			updateToolbar();
		});
	});
	...
	$discardBtn.on('click', function () {
		if (!confirm(wbe.i18n.confirmDiscard)) return;
