...
#
# Produces: <out>/october-popups-<version>.zip
#
# The repo app folder is `dev/oc-popups/` (matches docs/), but the WordPress
# plugin slug is `october-popups` (its main file / text domain). The zip is
# staged so its top-level folder is the WP slug, which is what WordPress
# installs and what the self-updater expects.
#
# Usage: bin/build-zip.sh [output_dir]
set -euo pipefail

PLUGIN_SLUG="october-popups"
