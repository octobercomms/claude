...
final class Volunteers {
    ...
    public function init(): void {
        ...
        add_shortcode('adf_volunteer_signup', [$this, 'render_signup_widget']);
    ...
    public function add_meta_box(): void {
        add_meta_box(
            'adf_volunteer_shifts',
            __('ADF — Role, location & shifts', 'adf-festival'),
    ...
    public function render_meta_box(\WP_Post $post): void {
        wp_nonce_field('adf_save_volunteer', 'adf_volunteer_nonce');
        $role     = (string) get_post_meta($post->ID, '_adf_role', true);
        $location = (string) get_post_meta($post->ID, '_adf_location', true);
        $open     = get_post_meta($post->ID, '_adf_signups_open', true);
        ...
        ?>
        <p><label><strong><?php esc_html_e('Role', 'adf-festival'); ?></strong><br>
            <input type="text" name="adf_role" class="widefat" value="<?php echo esc_attr($role); ?>" placeholder="e.g. Meet &amp; Greet Host"></label></p>
        <p><label><strong><?php esc_html_e('Location', 'adf-festival'); ?></strong><br>
            <input type="text" name="adf_location" class="widefat" value="<?php echo esc_attr($location); ?>"></label></p>
        <p><label><input type="checkbox" name="adf_signups_open" value="1" <?php checked($open, 1); ?>> <?php esc_html_e('Signups open', 'adf-festival'); ?></label></p>
        <p><label><strong><?php esc_html_e('Shifts', 'adf-festival'); ?></strong> — <?php esc_html_e('one per line:', 'adf-festival'); ?>
            <code>Label | start datetime | end datetime | capacity</code></label></p>
        <textarea name="adf_shifts" rows="5" class="widefat" placeholder="Sun Sept 28 — 10:30am–1:00pm | 2026-09-28 10:30 | 2026-09-28 13:00 | 3"><?php echo esc_textarea(implode("\n", $lines)); ?></textarea>
        <p class="description"><?php esc_html_e('Start/end accept any clear date-time (used to schedule reminders). Changing a shift label keeps existing signups attached.', 'adf-festival'); ?></p>
        <?php
    }

    public function save_meta(int $post_id): void {
        if (! isset($_POST['adf_volunteer_nonce']) || ! wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['adf_volunteer_nonce'])), 'adf_save_volunteer')) {
            return;
        }
        ...
        update_post_meta($post_id, '_adf_role', sanitize_text_field((string) ($_POST['adf_role'] ?? '')));
        update_post_meta($post_id, '_adf_location', sanitize_text_field((string) ($_POST['adf_location'] ?? '')));
        update_post_meta($post_id, '_adf_signups_open', empty($_POST['adf_signups_open']) ? '0' : '1');
        ...
        $raw = (string) wp_unslash($_POST['adf_shifts'] ?? '');
        $shifts = [];
        foreach (preg_split('/\r\n|\r|\n/', $raw) as $line) {
            $line = trim($line);
            if ($line === '') {
    ...
    public function register_meta(): void {
        ...
        foreach ([
            '_adf_role'         => 'string',
            '_adf_location'     => 'string',
            '_adf_signups_open' => 'boolean',
        ] as $key => $type) {
    ...
    public static function shifts(int $opportunity_id): array {
        $shifts = get_post_meta($opportunity_id, '_adf_shifts', true);
    ...
    public static function set_shifts(int $opportunity_id, array $shifts): void {
        ...
        update_post_meta($opportunity_id, '_adf_shifts', $clean);
    }
    ...
    public static function signup(int $opportunity_id, string $shift_id, array $person, int $account_id = 0) {
        if (get_post_type($opportunity_id) !== self::slug()) {
            return new \WP_Error('adf_bad_opportunity', __('Unknown volunteer opportunity.', 'adf-festival'));
        }
        // Unset is treated as open; only an explicit "0" closes signups.
        if (get_post_meta($opportunity_id, '_adf_signups_open', true) === '0') {
            return new \WP_Error('adf_signups_closed', __('Signups for this opportunity are closed.', 'adf-festival'));
        }
        $shift = self::shift($opportunity_id, $shift_id);
        if (! $shift) {
            return new \WP_Error('adf_bad_shift', __('That shift no longer exists.', 'adf-festival'));
        }
        if (self::shift_full($opportunity_id, $shift_id)) {
            return new \WP_Error('adf_shift_full', __('Sorry — that shift is full.', 'adf-festival'));
        }

        $name  = sanitize_text_field((string) ($person['name'] ?? ''));
        $email = sanitize_email((string) ($person['email'] ?? ''));
        if ($name === '' || ! is_email($email)) {
            return new \WP_Error('adf_invalid_person', __('A name and valid email are required.', 'adf-festival'));
        }
        ...
        foreach (VolunteerSignups::for_shift($opportunity_id, $shift_id) as $existing) {
            if (strcasecmp($existing->email, $email) === 0 && $existing->status !== VolunteerSignups::STATUS_DECLINED) {
                return new \WP_Error('adf_already_booked', __('You are already signed up for this shift.', 'adf-festival'));
            }
        }
        ...
        if (isset($lists['adf_volunteers'])) {
            BrevoConnector::upsert_contact($email, ['FIRSTNAME' => $name], [(int) $lists['adf_volunteers']]);
        }
    ...
    public static function email_params(object $signup): array {
        ...
        return [
            ...
            'shift'       => $shift['label'] ?? '',
            'location'    => (string) get_post_meta((int) $signup->opportunity_id, '_adf_location', true),
    ...
    public static function sms_body(object $signup, string $context): string {
        ...
        if ($context === 'on_signup') {
            return sprintf(
                ...
                __('ADF: thanks %1$s! You are signed up to volunteer for %2$s (%3$s). Reply STOP to opt out.', 'adf-festival'),
                $p['name'],
                $p['opportunity'],
                $p['shift']
            );
        ...
        return sprintf(
            ...
            __('ADF reminder: your volunteer shift "%1$s" (%2$s) is coming up at %3$s. See you there!', 'adf-festival'),
            $p['opportunity'],
            $p['shift'],
            $p['location']
        );
    ...
    /**
     * `[adf_volunteer_signup opportunity="123"]` — renders the shift table with
     * remaining slots and a signup form, hydrated against the REST API.
     */
    public function render_signup_widget(array $atts = []): string {
        $atts = shortcode_atts(['opportunity' => get_the_ID()], $atts, 'adf_volunteer_signup');
        ...
        wp_localize_script('adf-dashboard', 'ADF_VOL', [
        ...
        return '<div class="adf-vol-signup" id="adf-vol-signup" data-opportunity="' . esc_attr((string) $opportunity_id) . '"></div>';
    }
