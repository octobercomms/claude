 ...
 *
 * The key is derived from a dedicated `OE_ENCRYPTION_KEY` constant if defined,
...
final class Crypto {
    ...
    private static function key(): string {
        if (defined('OE_ENCRYPTION_KEY') && OE_ENCRYPTION_KEY) {
            return hash('sha256', (string) OE_ENCRYPTION_KEY, true);
        }
        $salt = (defined('AUTH_KEY') ? AUTH_KEY : '') . (defined('SECURE_AUTH_SALT') ? SECURE_AUTH_SALT : '');
        // NOTE: literal kept stable across the ADF→October Events rename so secrets
        // encrypted before the rename still decrypt. Do not change it.
