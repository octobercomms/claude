...
class OCF_Mail {
	...
	public static function ses_enabled() {
		return (bool) get_option( 'ocf_ses_enabled', false );
	}

	public static function configure_phpmailer( $phpmailer ) {
		if ( ! self::ses_enabled() ) { return; }

		$region   = trim( (string) get_option( 'ocf_ses_region', 'us-east-1' ) );
		$username = trim( (string) get_option( 'ocf_ses_smtp_username', '' ) );
		$password = trim( (string) get_option( 'ocf_ses_smtp_password', '' ) );
		$port     = (int) get_option( 'ocf_ses_smtp_port', 587 );
