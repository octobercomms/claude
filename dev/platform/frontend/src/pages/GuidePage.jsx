...
const SECTIONS = [
  ...
  },
  {
    ...
    body: [
      ...
      '**API spend this month** banner (admin) — combined pay-per-use spend plus each provider\'s remaining **balance / quota** (e.g. DataForSEO credit, Hunter searches), so you catch a low balance before it bites. **Breakdown →** opens Settings → Costs & usage. (A live £/$ spend figure needs an Anthropic Admin key — see Settings.)',
  ...
  },
  {
    id: 'setup',
    title: 'Client → Setup',
    summary: 'The brief, brand kit and connectors — set once, used everywhere.',
    body: [
      'Four tabs, opening on **Overview** (a quick map of what each part does): **Overview · Brief · Brand · Connectors**.',
      '**Brief** — client name, slug, **domain** (required for SEO, AI-Overview tracking and any "research this brand" action), an active/inactive status toggle, and the **About this client** paragraph Claude reads on every report, social batch, ad concept and chat reply. Be specific — a weak brief = generic output.',
      '**Complete with Claude** researches the business from its domain and drafts the About paragraph for you.',
      '**Brand** and **Connectors** each have their own section below.',
      'Report **recipients + schedule** live on the **Reports** tab; the **ads gross-margin** assumption lives on the **Paid** page.',
    ],
  },
  {
    id: 'client-ai',
  ...
  },
  {
    ...
    body: [
      ...
      '**AI Visibility tab (GEO)** — share-of-voice across Claude, ChatGPT, Gemini, Perplexity and Google AI Overviews for the prompts users actually ask in your category. The new battleground beyond classic blue-link SEO. (Tabs are grouped **SEO · GEO · Content** at the top.)',
  ...
  },
  {
    ...
    body: [
      'Five tabs: **Overview · Performance · Strategist · Creative · Audiences**. Google + Meta render on the one Performance page, stacked.',
      '**Performance tab** = live Google Ads + Meta Ads numbers (spend, revenue, ROAS, profit). The gross-margin field flows into the profit calcs.',
      '**Strategist tab** — a weekly Manus-style analyst brief: Claude reads this period vs. the previous one and writes the to-do list (pause this campaign, shift budget there). Auto-generated Monday 07:00, emailed to a recipient list, exportable as PDF. The Summary Scorecard has tick-off checkboxes.',
      '**Creative tab** generates direct-response ad concepts (PAS / AIDA / Before-After / Social Proof / FOMO) — headline + body + CTA + visual concept, grounded in the brand assets. **Render images** via Replicate / Ideogram / Adobe Firefly across aspect ratios; hover an image → **↔** fans it to every other ratio via Adobe generative resize. **Share for approval** gives the client a no-login review link.',
      '**Audiences tab** — build targetable segments from Shopify postcode data **or an uploaded customer-list CSV** (emails/phones hashed on upload, never stored raw), exported as Meta Custom Audience CSVs.',
      '**Overview tab** carries the four-layer Google Ads account-structure playbook (Performance Max → Standard Shopping → Standard Search → Branded) as a reference funnel with the do / don\'t for each layer.',
    ],
  },
  {
    id: 'social',
  ...
  },
  {
    ...
    body: [
      ...
      '**Contacts** — find via Hunter + Serper, import a CSV, or pull from the shared workspace library; tag by location, role, contact type. All the actions sit in one toolbar.',
      '**Campaigns** — define the angle and audience filter; Claude drafts the sequence (subject + body per step). **Launch** sends step 1 and the scheduler handles follow-ups. Replies hit the IMAP inbox, get classified by Claude, and auto-unsubscribe matching contacts.',
      '**Tasks** — when a campaign uses a **multichannel sequence** (LinkedIn visit → connect → message, or a manual step), each step lands here as a daily queue. Tick one off and the prospect advances to the next step automatically, exactly like a sent email.',
      '**Sending** — rotate multiple sender mailboxes, each with its own daily cap + warm-up state, so no single inbox trips provider limits. (Falls back to a single-sender config if none are added.)',
      'Setup keys + DNS records for this suite are in the next section.',
    ],
  },
  {
    id: 'outreach-setup',
  ...
  },
  {
    ...
    body: [
      'Tabs: **General · Contacts · Tags · Users & access**.',
      '**Costs & usage** panel at the top of General — latest balance / monthly spend per pay-per-use provider, colour-coded green/amber/red. Auto-refreshes nightly at 02:00; "Refresh now" forces a poll. A panel showing "Connected — add an Admin key" means the feature works but spend can\'t be read yet.',
      '**Platform keys** by category (AI & Email, Ad Platforms, Ecommerce & Inventory, SEO, Outreach, Other) — each block has its own Save button. OAuth scopes for Shopify + Meta are shown inline with copy-all buttons.',
      'For Anthropic **spend tracking**, add an `ANTHROPIC_ADMIN_KEY` alongside the regular Claude key — it\'s the only provider that exposes an actual £/$ spend figure.',
      'Change **your own password** from the sidebar footer (**Change password**) — it\'s available on every page, not just here.',
    ],
  },
  {
    id: 'users',
    title: 'Settings → Users & access (admin only)',
    summary: 'Add and remove users; control who sees which clients.',
    body: [
      'On the **Users & access** tab of Settings. **+ Add user** — username, password, role (admin or viewer), assigned clients.',
      'Viewer role sees only the clients you tick. Admin sees everything. Admins can also rename their own username here.',
  ...
  },
  {
    ...
    body: [
      ...
      '**Deep-link any tab.** The active tab lives in the URL (`?tab=…`), so you can bookmark or share a link straight to, say, a client\'s Strategist brief or Competitors view.',
      '**It works on your phone.** The sidebar collapses into a top-bar menu; the per-client pages, tables and chat all reflow for mobile.',
    ],
  },
];
