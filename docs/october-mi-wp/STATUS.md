...
## v1.8.0 — Mode-first UX, managed images, robust parsing (from live testing)

Driven by first live-test feedback.

- **Standalone vs Integrated is now the primary choice** and it drives the UI. Replaced the
  separate `connect_enabled` + `key_source` controls with one `run_mode`
  (`standalone` | `integrated`). Standalone shows the key fields; **integrated hides them
  entirely** — Claude generation and image generation are pulled from the platform, so the
  only thing entered is a pairing token.
- **Managed images**: in integrated mode the hero is generated by the platform (OMI's
  fal.ai) via a new `POST /api/wp-connect/image` call (accepts base64 or URL; 404 = not-yet-
  shipped → skips quietly). Standalone still uses the site's own Gemini key. Added the image
  endpoint to `OMI-CONNECTION-BRIEF.md`.
- **Robust model-reply parsing** (fixes "The model did not return a usable topic plan"):
  a shared tolerant JSON extractor (code fences, bare arrays, alt keys) + tolerant handling
  of the platform's response envelope, used by the planner, writer, and site-learner. On
  failure they now log and surface a short excerpt of the raw reply for diagnosis.
