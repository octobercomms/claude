...
class OCF_Analytics_Page {
	...
	public static function render() {
		...
		echo '<input type="hidden" name="page" value="oc-forms-analytics">';
		...
		$stats   = OCF_Analytics::form_stats( $form_id, $from, $to );
		$funnel  = OCF_Analytics::funnel( $form_id, $from, $to );
		$series  = OCF_Analytics::timeseries( $form_id, $from, $to );
		...
		echo '<a class="button" href="' . esc_url( admin_url( 'admin.php?page=oc-forms-submissions&form_id=' . $form_id ) ) . '">View submissions →</a> ';
		echo '<a class="button" href="' . esc_url( wp_nonce_url( admin_url( 'admin.php?page=oc-forms-submissions&form_id=' . $form_id . '&action=export' ), 'ocf_export' ) ) . '">Export CSV</a>';
		echo '</p>';
	...
	private static function render_kpis( $stats ) {
		...
		echo '<style>
			.ocf-kpi-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 12px; }
			.ocf-kpi { background: #fff; border: 1px solid #c3c4c7; border-radius: 4px; padding: 16px; }
			.ocf-kpi-label { font-size: 12px; color: #6c7781; text-transform: uppercase; letter-spacing: .04em; }
			.ocf-kpi-value { font-size: 28px; font-weight: 600; margin-top: 4px; }
			.ocf-card { background: #fff; border: 1px solid #c3c4c7; border-radius: 4px; padding: 20px; }
			.ocf-card h2 { margin-top: 0; }
			.ocf-bar { height: 26px; background: #2271b1; border-radius: 3px; display: flex; align-items: center; padding: 0 10px; color: #fff; font-size: 13px; min-width: 4px; transition: width .2s; }
			.ocf-funnel-row { display: grid; grid-template-columns: 200px 1fr 60px; gap: 12px; align-items: center; margin-bottom: 8px; }
			.ocf-funnel-row span:last-child { text-align: right; font-variant-numeric: tabular-nums; }
			.ocf-ts svg { width: 100%; height: 220px; display: block; }
		</style>';
	}

	private static function kpi( $label, $value ) {
		echo '<div class="ocf-kpi"><div class="ocf-kpi-label">' . esc_html( $label ) . '</div><div class="ocf-kpi-value">' . esc_html( $value ) . '</div></div>';
	}

	private static function render_funnel( $funnel ) {
		$max = 0;
		foreach ( $funnel as $row ) { if ( $row['reached'] > $max ) { $max = $row['reached']; } }
		echo '<div class="ocf-card"><h2>Step funnel</h2>';
		if ( ! $funnel || ! $max ) {
			...
			foreach ( $funnel as $row ) {
				...
				echo '<div class="ocf-funnel-row">';
				echo '<span>' . esc_html( $label ) . '</span>';
				echo '<div class="ocf-bar" style="width: ' . (int) $pct . '%;">' . (int) $pct . '%</div>';
	...
	private static function render_timeseries( $series ) {
		echo '<div class="ocf-card ocf-ts"><h2>Daily activity</h2>';
		if ( empty( $series ) ) {
