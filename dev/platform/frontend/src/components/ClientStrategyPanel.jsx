  ...
  return (
    <div className={`card ${cls}`} style={{ padding: 'var(--s4)' }}>
      ...
      <ul style={{ margin: 0, paddingLeft: 'var(--s5)' }}>
        {items.map((it, i) => <li key={i} className="body-sm" style={{ marginBottom: 'var(--s1)' }}>{it}</li>)}
      </ul>
    </div>
  );
}
...
function CompetitorCol({ title, blurb, items }) {
  ...
  return (
    <div className="card" style={{ padding: 'var(--s4)' }}>
      <div className="caption" style={{ marginBottom: 'var(--s1)' }}>{title}</div>
      <div className="body-xs text-subtle" style={{ marginBottom: 'var(--s2)' }}>{blurb}</div>
      <ul style={{ margin: 0, paddingLeft: 'var(--s5)' }}>
        {items.map((it, i) => <li key={i} className="body-sm" style={{ marginBottom: 'var(--s1)' }}>{it}</li>)}
      </ul>
    </div>
    ...
    <div className="stack stack-lg" style={{ marginBottom: 'var(--s7)' }}>
      {p.exec_summary && (
        <div className="card filled">
          <div className="caption" style={{ marginBottom: 'var(--s2)' }}>Executive summary</div>
      ...
      {(p.positioning || !!p.key_messages?.length) && (
        <div className="card accent">
          {p.positioning && (
            <>
              <div className="caption" style={{ marginBottom: 'var(--s2)' }}>Positioning</div>
              <p className="body" style={{ margin: '0 0 var(--s3)', fontWeight: 600 }}>{p.positioning}</p>
            </>
          )}
          {!!p.key_messages?.length && (
            <>
              <div className="caption" style={{ marginBottom: 'var(--s2)' }}>Key messages</div>
              <ul style={{ margin: 0, paddingLeft: 'var(--s5)' }}>
                {p.key_messages.map((m, i) => <li key={i} className="body-sm" style={{ marginBottom: 'var(--s1)' }}>{m}</li>)}
              </ul>
            </>
          )}
        </div>
          ...
          <div className="metric-grid">
            {p.objectives.map((o, i) => (
              <div key={i} className="card" style={{ padding: 'var(--s4)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 'var(--s2)', marginTop: 'var(--s2)', flexWrap: 'wrap' }}>
                ...
                {o.timeframe && <div className="body-xs text-subtle" style={{ marginTop: 'var(--s1)' }}>{o.timeframe}</div>}
              </div>
            ))}
          </div>
        </div>
      ...
      {!!p.personas?.length && (
        <div>
          <div className="caption" style={{ marginBottom: 'var(--s2)' }}>Audience</div>
          <div className="grid grid-auto">
            {p.personas.map((pe, i) => (
              <div key={i} className="card">
                <span className="chip chip-accent" style={{ fontSize: 'var(--fs-caption)' }}>{pe.label}</span>
                {pe.who && <p className="body-sm" style={{ margin: 'var(--s2) 0 0' }}>{pe.who}</p>}
      ...
      {hasSwot && (
        <div>
          <div className="caption" style={{ marginBottom: 'var(--s2)' }}>SWOT</div>
      ...
      {hasComp && (
        <div>
          <div className="caption" style={{ marginBottom: 'var(--s2)' }}>Competitor landscape</div>
      ...
      {!!p.competitor_table?.length && (
        <div>
          <div className="caption" style={{ marginBottom: 'var(--s2)' }}>Competitor benchmark</div>
          ...
          <div className="body-xs text-subtle" style={{ marginTop: 'var(--s1)' }}>DA = DataForSEO domain rank (0–1000), pulled live where available.</div>
        </div>
      )}

      {p.funnel && (p.funnel.attract.length || p.funnel.convert.length || p.funnel.close.length || p.funnel.retain.length) ? (
        <div>
          <div className="caption" style={{ marginBottom: 'var(--s2)' }}>Demand funnel — tactics by stage</div>
          <div className="grid grid-auto">
      ...
      {(!!p.target_media?.length || !!p.target_awards?.length) && (
        <div className="grid grid-2">
          {!!p.target_media?.length && (
            <div className="card" style={{ padding: 'var(--s4)' }}>
              <div className="caption" style={{ marginBottom: 'var(--s2)' }}>Target media</div>
              <div className="stack stack-sm">
                {p.target_media.map((m, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', gap: 'var(--s2)', alignItems: 'baseline' }}>
          ...
          {!!p.target_awards?.length && (
            <div className="card" style={{ padding: 'var(--s4)' }}>
              <div className="caption" style={{ marginBottom: 'var(--s2)' }}>Target awards</div>
              <div className="stack stack-sm">
    ...
    <div className="card" style={{ marginBottom: 'var(--s5)' }}>
      ...
      {showPicker ? (
        ...
        <>
          ...
          <div className="caption" style={{ marginBottom: 'var(--s3)' }}>The SOSTAC plan</div>
              ...
              return (
                <div key={pi} className="card" style={{ padding: 0, overflow: 'hidden', borderLeft: `5px solid ${s.color}` }}>
                  ...
                  <div className="stack stack-sm" style={{ padding: 'var(--s4) var(--s5)' }}>
                    {(ph.items || []).map(it => (
                      <div key={it.id} style={{ display: 'flex', gap: 'var(--s2)', alignItems: 'flex-start' }}>
                        <input type="checkbox" checked={!!it.done} onChange={e => toggle(it.id, e.target.checked)} style={{ marginTop: 'var(--s1)', accentColor: s.color }} />
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <input
                            className="input"
                            style={{ marginTop: 'var(--s1)', fontSize: 'var(--fs-caption)', padding: 'var(--s1) var(--s2)' }}
