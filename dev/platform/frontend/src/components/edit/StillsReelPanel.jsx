    ...
    <div className="stack" style={{ gap: 'var(--s4)' }}>
      ...
      {images.length === 0 ? (
        <div
          ...
          <div style={{ fontSize: 'var(--fs-body)', color: 'var(--text-subtle)', marginTop: 'var(--s2)' }}>Each still is animated into a short cinematic clip, then stitched into one reel</div>
        </div>
      ) : (
        <div className="edit-split"
          ...
          <div>
            ...
            <button className="btn btn-secondary btn-sm" style={{ marginTop: 'var(--s3)' }} onClick={() => fileRef.current?.click()} disabled={images.length >= 12}>+ Add still</button>
          </div>

          {/* RIGHT — settings */}
          <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--s4)', position: 'sticky', top: 16 }}>
            <div>
              <label className="caption" style={{ display: 'block', marginBottom: 'var(--s2)' }}>Camera motion</label>
              ...
              <div style={{ fontSize: 'var(--fs-caption)', color: 'var(--text-subtle)', marginTop: 'var(--s1)' }}>Applied to every still — subtle moves montage best.</div>
            </div>

            <div>
              <label className="caption" style={{ display: 'block', marginBottom: 'var(--s2)' }}>Shape</label>
            ...
            <div>
              <label className="caption" style={{ display: 'block', marginBottom: 'var(--s2)' }}>Each clip: {perClip.toFixed(1)}s</label>
              <input type="range" min="0.6" max="4" step="0.1" value={perClip} onChange={e => setPerClip(Number(e.target.value))} style={{ width: '100%' }} />
              <div style={{ fontSize: 'var(--fs-caption)', color: 'var(--text-subtle)', marginTop: 'var(--s1)' }}>Reel length ~{reelSeconds.toFixed(1)}s across {images.length} clip{images.length === 1 ? '' : 's'}.</div>
            </div>

            <div style={{ fontSize: 'var(--fs-caption)', color: 'var(--text-muted)', background: 'var(--surface)', borderRadius: 'var(--r-sm)', padding: 'var(--s3)' }}>
