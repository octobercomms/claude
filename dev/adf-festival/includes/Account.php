...
/**
 * Account model (§1.2 `adf_account`).
 *
 * Each account is an `adf_account` post linked to a wp_user. Holds the
...
final class Account {
    ...
    public static function create(int $wp_user_id): int {
        ...
        $defaults = [
            '_adf_wp_user_id'       => $wp_user_id,
            '_adf_organisation_name'=> '',
            '_adf_contact_name'     => $name,
            '_adf_email'            => $user->user_email,
            '_adf_phone'            => '',
            '_adf_billing_address'  => '',
            '_adf_stripe_customer_id'=> '',
            '_adf_auto_approve'     => 0,
            '_adf_auto_approve_types'=> [],
            '_adf_account_status'   => 'active',
            '_adf_created_date'     => current_time('mysql'),
        ];
        ...
        ], isset($lists['adf_all_subscribers']) ? [(int) $lists['adf_all_subscribers']] : []);
    ...
    public static function auto_approves(int $account_id, string $listing_type): bool {
        ...
        if ((string) get_post_meta($account_id, '_adf_account_status', true) === 'suspended') {
            return false;
        }
        if (! get_post_meta($account_id, '_adf_auto_approve', true)) {
            return false;
        }
        $types = (array) get_post_meta($account_id, '_adf_auto_approve_types', true);
    ...
    public static function email(int $account_id): string {
        return (string) get_post_meta($account_id, '_adf_email', true);
    }

    public static function name(int $account_id): string {
        return (string) get_post_meta($account_id, '_adf_contact_name', true) ?: get_the_title($account_id);
    }
}
