...
export default function PressReviewPage() {
  ...
  const card = { background: '#fff', border: '1px solid #e5e7eb', borderRadius: 14, padding: 'var(--s7)', boxShadow: '0 1px 3px rgba(0,0,0,.04)' };
    ...
    <div style={{ background: '#f7f7f8', minHeight: '100vh' }}>
      <div style={wrap}>
        <div style={card}>
          ...
          {approved ? (
            <p style={{ display: 'inline-block', fontSize: 'var(--fs-body)', padding: 'var(--s2) var(--s3)', borderRadius: 20, background: '#dcfce7', color: '#166534' }}>
            ...
            <>
              <div style={{ display: 'flex', gap: 'var(--s3)', alignItems: 'center', flexWrap: 'wrap' }}>
                <input value={approver} onChange={(e) => setApprover(e.target.value)} placeholder="Your name" style={{ padding: 'var(--s2) var(--s3)', border: '1px solid #d1d5db', borderRadius: 8, fontSize: 'var(--fs-body)' }} />
                <button onClick={approve} disabled={submitting} style={{ background: '#166534', color: '#fff', border: 'none', padding: 'var(--s3) var(--s5)', borderRadius: 8, cursor: 'pointer', fontSize: 'var(--fs-body)' }}>{submitting ? 'Approving…' : 'Approve this release'}</button>
              </div>
