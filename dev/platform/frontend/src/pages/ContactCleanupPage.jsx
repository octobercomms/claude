...
export default function ContactCleanupPage() {
  ...
  async function mergeOne(ci) {
    ...
    if (!canon) { toast('Pick which journalist to keep', 'error'); return; }
    if (!confirm(`Merge ${cluster.members.length - 1} duplicate${cluster.members.length === 2 ? '' : 's'} into the selected journalist? Cannot be undone.`)) return;
    ...
    try {
      ...
      toast(`Merged ${r.merged} journalist${r.merged === 1 ? '' : 's'}`, 'success');
    } catch (e) { toast(e.message, 'error'); }
    finally { setBusy(false); }
  }

  async function deleteRow(ci, memberId, memberName) {
    if (!confirm(`Delete "${memberName}" outright? This wipes the journalist and every history row attached to it — not the same as merging.`)) return;
    setBusy(true);
    try {
  ...
  return (
    <div className="suite-cleanup">
      ...
      <header className="hero" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 12, flexWrap: 'wrap' }}>
        ...
        <Link to="/settings?tab=contacts" className="btn btn-secondary">← Back to journalists</Link>
      </header>

      <SuiteTabs tabs={[
        { groupLabel: 'Journalists' },
          ...
          <div className="card" style={{ marginBottom: 16, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            <div style={{ fontSize: 13, color: 'var(--text-muted)', flex: 1, minWidth: 240 }}>
              {scanning
                ...
                : clusters
                  ? `${remaining.length} matchup${remaining.length === 1 ? '' : 's'} to review${remaining.length ? ' · merging folds the coverage history onto the journalist with the email' : ''}`
          ...
          {cluster.members.map((m) => (
            <tr key={m.id}>
              ...
              <td style={{ textAlign: 'right' }}>
                <button className="btn btn-danger btn-sm" onClick={() => onDelete(m.id, m.name)} disabled={busy} title="Delete this journalist outright (not a merge)">Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
...
function TidyFixesTab({ onChanged }) {
  ...
  async function repairImportedNames() {
    if (!confirm('Strip leftover Notion-URL fragments from every journalist + outlet name? Safe to run multiple times.')) return;
    setRepairing(true);
    try {
      const r = await api.post('/pr/repair-imported-names', {});
      toast(`Repaired ${r.contacts} journalist${r.contacts === 1 ? '' : 's'} and ${r.outlets} outlet${r.outlets === 1 ? '' : 's'}.`, 'success');
      if (onChanged) onChanged();
    } catch (e) { toast(e.message, 'error'); }
