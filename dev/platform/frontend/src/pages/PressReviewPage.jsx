...
export default function PressReviewPage() {
  ...
  return (
    <div style={{ background: '#f7f7f8', minHeight: '100vh' }}>
      <div style={wrap}>
        <div style={card}>
          <h1 style={{ margin: '0 0 2px', fontSize: 'var(--fs-section)' }}>{data.title}</h1>
          <p style={{ margin: '0 0 18px', color: '#6b7280', fontSize: 'var(--fs-body)' }}>{data.client_name ? `${data.client_name} · ` : ''}Press release for approval</p>
          ...
          {approved ? (
            <p style={{ display: 'inline-block', fontSize: 'var(--fs-body)', padding: '6px 12px', borderRadius: 20, background: '#dcfce7', color: '#166534' }}>
              ✓ Approved{data.approved_by ? ` by ${data.approved_by}` : ''}{data.approved_at ? ` on ${fmtDate(data.approved_at)}` : ''}
            </p>
          ) : (
            <>
              <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
                <input value={approver} onChange={(e) => setApprover(e.target.value)} placeholder="Your name" style={{ padding: '8px 12px', border: '1px solid #d1d5db', borderRadius: 8, fontSize: 'var(--fs-body)' }} />
                <button onClick={approve} disabled={submitting} style={{ background: '#166534', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: 8, cursor: 'pointer', fontSize: 'var(--fs-body)' }}>{submitting ? 'Approving…' : 'Approve this release'}</button>
              </div>
              <p style={{ fontSize: 'var(--fs-body)', color: '#6b7280', marginTop: 10 }}>Spotted something? Reply to the email this link came from and we'll revise it.</p>
            </>
          )}
        </div>
        <p style={{ textAlign: 'center', color: '#6b7280', fontSize: 'var(--fs-caption)', marginTop: 18 }}>October Comms.</p>
      </div>
    </div>
  );
}
