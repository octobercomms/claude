...
async function analyseBatch(client, contacts) {
  ...
  const prompt = [
    ...
    '',
    'Publication-stuffed names — these come from media-list CSVs where the importer crammed multiple fields into `name`. Detect these patterns and propose the cleanup:',
    '- "Surname, Firstname - Publication - Section"     →  name="Firstname Surname", company="Publication", title="Section editor" (or leave title if unclear)',
    '- "Surname Firstname - Publication" / "Surname, Firstname (Publication)" → same idea',
    '- "Publication - Firstname Lastname"               →  name="Firstname Lastname", company="Publication"',
    '- "Publication\'s Info & Sales Team"               →  name="Info & Sales Team", company="Publication" (these are role accounts, not people)',
    '- A bare publication / section label as `name` with no person ("Trends Edit", "ULM", "The Plan") + a role-account email (info@, mail@, editorial@) → propose company=publication-name and leave first_name/last_name empty (it isn\'t a person)',
    'When proposing these splits, emit one suggestion per field changed (name, first_name, last_name, company, title) so the AM can tick each independently.',
    '',
