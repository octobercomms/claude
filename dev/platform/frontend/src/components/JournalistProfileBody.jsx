...
// The editable journalist "card": name, publication, email, beats, availability,
// notes, plus their learned topics, latest articles and coverage history. Shared
// by the full-page route (JournalistProfilePage) and the in-place ProfileModal so
// there's one source of truth. `mode` swaps the page hero for a compact modal
// header; onClose/onDeleted/onSaved let the host decide navigation + refresh.
...
export default function JournalistProfileBody({ id, mode = 'page', onClose, onDeleted, onSaved }) {
  ...
  async function save() {
    ...
    try {
      ...
      toast('Saved', 'success'); load(); onSaved?.();
  ...
  const title = `${c.first_name || ''} ${c.last_name || ''}`.trim() || 'Journalist';
  return (
    <>
