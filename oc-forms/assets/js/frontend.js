...
(function () {
	...
	function Form(root) {
		...
		var progress = el('div', { class: 'ocf-progress' }, [el('div', { class: 'ocf-progress-bar' })]);
		var stepHost = el('div', { class: 'ocf-step-host' });
		var foot = el('div', { class: 'ocf-foot' });
		...
		function renderProgress() {
			...
			var bar = progress.querySelector('.ocf-progress-bar');
		...
		function renderStep() {
			...
			if (schema.spam && schema.spam.honeypot) {
				var hp = el('div', { class: 'ocf-hp', 'aria-hidden': 'true', style: { position: 'absolute', left: '-9999px', height: '0', overflow: 'hidden' } }, [
					el('label', null, ['Leave this empty', el('input', { type: 'text', name: 'ocf_hp', tabindex: '-1', autocomplete: 'off' })]),
					el('label', null, ['Email', el('input', { type: 'email', name: 'ocf_hp_email', tabindex: '-1', autocomplete: 'off' })])
				]);
			...
			if (isLast && schema.spam && schema.spam.turnstile && cfg.turnstileKey) {
				var ts = el('div', { class: 'ocf-turnstile', id: 'ocf-ts-' + cfg.formId });
				foot.appendChild(ts);
				mountTurnstile(ts);
			}
			var nextBtn = el('button', { type: 'button', class: 'ocf-btn ocf-btn-primary', onClick: isLast ? submit : next },
		...
		function showErrors(errors) {
			Array.from(stepHost.querySelectorAll('.ocf-q-error')).forEach(function (n) { n.remove(); });
			Array.from(stepHost.querySelectorAll('.ocf-q.has-error')).forEach(function (n) { n.classList.remove('has-error'); });
			errors.forEach(function (e) {
				var host = stepHost.querySelector('[data-q="' + e.id + '"]');
				if (host) {
					host.classList.add('has-error');
					host.appendChild(el('div', { class: 'ocf-q-error' }, [e.msg]));
				}
			});
		}
		...
		function renderEnding(ending) {
			...
			stepHost.appendChild(el('div', { class: 'ocf-ending' }, [
		...
		function renderQuestion(q) {
			var wrap = el('div', { class: 'ocf-q ocf-q-' + q.type, 'data-q': q.id });
			if (q.label && q.type !== 'heading') {
				wrap.appendChild(el('label', { class: 'ocf-q-label', html: q.label + (q.required ? ' <span class="ocf-req">*</span>' : '') }));
			}
			if (q.help) wrap.appendChild(el('div', { class: 'ocf-q-help', html: q.help }));

			var body = el('div', { class: 'ocf-q-body' });
			switch (q.type) {
				case 'heading':     body.appendChild(el('h2', { class: 'ocf-heading' }, [q.label || ''])); break;
				case 'paragraph':   body.appendChild(el('div', { class: 'ocf-paragraph', html: q.help || q.label || '' })); break;
		...
		function textInput(q, type) {
			var input = el('input', {
				type: type, class: 'ocf-input', value: answers[q.id] || '', placeholder: q.placeholder || '',
		...
		function textArea(q) {
			return el('textarea', {
				class: 'ocf-input ocf-textarea', placeholder: q.placeholder || '',
				rows: 5,
				onInput: function (e) { setAnswerDebounced(q.id, e.target.value); }
			}, [answers[q.id] || '']);
		}
		...
		function choiceList(q, multi) {
			var host = el('div', { class: 'ocf-choices' });
			(q.options || []).forEach(function (opt) {
				...
				host.appendChild(el('label', { class: 'ocf-choice' + (checked ? ' is-checked' : '') }, [input, el('span', null, [opt.label])]));
			});
			return host;
		}

		function imageCards(q, multi) {
			var host = el('div', { class: 'ocf-image-cards' });
			(q.options || []).forEach(function (opt) {
				var checked = multi ? (Array.isArray(answers[q.id]) && answers[q.id].indexOf(opt.value) > -1)
					: answers[q.id] === opt.value;
				var input = el('input', {
				...
				var card = el('label', { class: 'ocf-image-card' + (checked ? ' is-selected' : ''),
					onClick: function (e) {
						e.preventDefault();
						if (multi) {
							var arr = Array.isArray(answers[q.id]) ? answers[q.id].slice() : [];
					...
					opt.image ? el('div', { class: 'ocf-image-card-img' }, [el('img', { src: opt.image, alt: opt.label || '' })]) : null,
					el('div', { class: 'ocf-image-card-meta' }, [input, el('span', null, [opt.label || ''])])
				]);
		...
		function dropdown(q) {
			...
			var sel = el('select', { class: 'ocf-input ocf-select', onChange: function (e) { setAnswer(q.id, e.target.value); } });
		...
		function grid(q) {
			...
			var table = el('table', { class: 'ocf-grid' });
			var thead = el('thead');
			...
			rows.forEach(function (row) {
				...
				cols.forEach(function (col) {
					...
					var input = el('input', { type: 'radio', name: 'ocf_' + q.id + '_' + row, value: col,
		...
		function addressGroup(q) {
			...
			function field(name, ph, cls) {
				return el('input', { type: 'text', class: 'ocf-input ' + (cls || ''), placeholder: ph, value: v[name] || '',
					onInput: function (e) { v[name] = e.target.value; setAnswer(q.id, v); } });
			}
			return el('div', { class: 'ocf-address' }, [
				field('line1', 'Address line 1'),
				field('line2', 'Address line 2'),
				el('div', { class: 'ocf-address-row' }, [
					field('city',    'City'),
		...
		function fileGroup(q) {
			var host = el('div', { class: 'ocf-file' });
			uploads[q.id] = uploads[q.id] || [];

			var input = el('input', { type: 'file', class: 'ocf-file-input',
			...
			var drop = el('div', { class: 'ocf-dropzone',
			...
			}, ['Drag & drop a file or ', el('span', { class: 'ocf-file-browse' }, ['browse'])]);
			var list = el('div', { class: 'ocf-file-list' });
		...
		function fileRow(q, info, list) {
			var row = el('div', { class: 'ocf-file-row' }, [
				el('span', { class: 'ocf-file-name' }, [info.name]),
				el('button', { type: 'button', class: 'ocf-file-remove', onClick: function () {
		...
		function uploadOne(q, file, list) {
			var row = el('div', { class: 'ocf-file-row is-uploading' }, [
				el('span', { class: 'ocf-file-name' }, [file.name]),
				el('span', { class: 'ocf-file-progress' }, ['Uploading…'])
			]);
	...
	function boot() {
		Array.from(document.querySelectorAll('.ocf-form-root')).forEach(function (root) {
			if (root.getAttribute('data-ocf-booted')) return;
			root.setAttribute('data-ocf-booted', '1');
