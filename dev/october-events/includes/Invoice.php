...
final class Invoice {
    ...
    public static function create(int $listing_id, string $payment_intent_id): void {
        if (get_post_meta($listing_id, '_oe_invoice', true)) {
        ...
        update_post_meta($listing_id, '_oe_invoice', $invoice);
    ...
    public static function for_account(int $account_id): array {
        $posts = get_posts([
            ...
            'meta_query'     => [
                ...
                ['key' => '_oe_invoice', 'compare' => 'EXISTS'],
            ],
        ]);
        $out = [];
        foreach ($posts as $pid) {
            $invoice = get_post_meta($pid, '_oe_invoice', true);
            if (is_array($invoice)) {
    ...
    private static function next_number(): string {
        $seq = (int) get_option('oe_invoice_seq', 1000);
        $seq++;
        update_option('oe_invoice_seq', $seq);
    ...
    public static function render_html(array $invoice): string {
        ...
        <h1><?php echo esc_html(get_bloginfo('name')); ?> — <?php esc_html_e('Invoice', 'october-events'); ?></h1>
        ...
        <table>
            <tr><th><?php esc_html_e('Item', 'october-events'); ?></th><th class="right"><?php esc_html_e('Amount', 'october-events'); ?></th></tr>
            <tr>
            ...
            <tr><td class="right"><strong><?php esc_html_e('Total', 'october-events'); ?></strong></td><td class="right"><strong><?php echo esc_html($amount); ?></strong></td></tr>
        </table>
        </body></html>
        <?php
