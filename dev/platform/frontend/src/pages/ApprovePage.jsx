...
export default function ApprovePage() {
  ...
  if (err) return (
    <div className="suite-social" style={{ padding: 'var(--s7) var(--s5)' }}>
      <div className="callout callout-danger" style={{ maxWidth: 600, margin: 'var(--s8) auto' }}>{err}</div>
    </div>
  );
  if (!data) return (
    <div className="suite-social" style={{ padding: 'var(--s7) var(--s5)' }}>
      <div className="text-subtle" style={{ padding: 'var(--s8)', textAlign: 'center' }}>Loading…</div>
    </div>
  );
  ...
  return (
    <div className="suite-social" style={{ padding: 'var(--s7) var(--s5)', maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div className="field" style={{ display: 'flex', alignItems: 'center', gap: 'var(--s3)', maxWidth: 400 }}>
...
function ResponsesList({ responses }) {
  ...
  return (
    <div className="mt-4" style={{ paddingTop: 'var(--s3)', borderTop: 'var(--border-w) solid var(--accent-soft)' }}>
      {responses.map((r, i) => {
        ...
        return (
          <div key={i} className="mb-3">
            ...
            {r.comment && <div className="body-sm mt-2" style={{ padding: 'var(--s2) var(--s2)', background: 'var(--surface-raised)', borderRadius: 'var(--r-sm)' }}>{r.comment}</div>}
          </div>
        );
      })}
    </div>
  ...
  return (
    <div className="card">
      ...
      {(post.image_urls || []).length > 0 && (
        <div className="grid" style={{ marginBottom: 'var(--s3)', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 'var(--s2)' }}>
          {post.image_urls.map((u, i) => <img key={i} src={u} alt="" style={{ width: '100%', borderRadius: 'var(--r-sm)' }} />)}
        </div>
      )}
      {(post.media || []).filter(m => m.kind === 'video').map(v => (
        <video key={v.id} src={v.url} controls style={{ width: '100%', borderRadius: 'var(--r-sm)', marginBottom: 'var(--s2)', background: '#000' }} />
      ))}
      {(post.media || []).filter(m => m.kind === 'audio').map(a => (
        <audio key={a.id} src={a.url} controls style={{ width: '100%', marginBottom: 'var(--s2)' }} />
      ))}
      <div className="h3 mb-2">{post.hook}</div>
      <div className="body-sm" style={{ whiteSpace: 'pre-wrap' }}>{post.caption}</div>
      {(post.hashtags || []).length > 0 && (
        <div className="mt-2">
          {post.hashtags.map(h => <span key={h} className="text-accent" style={{ fontSize: 'var(--fs-caption)', marginRight: 'var(--s2)' }}>#{h.replace(/^#/, '')}</span>)}
        </div>
      )}
  ...
  return (
    <div className="card">
      ...
      {(creative.images || []).length > 0 && (
        <div className="row wrap" style={{ marginBottom: 'var(--s3)', gap: 'var(--s2)' }}>
