<?php
/**
 * Self-updater for the Bulk Editor, distributed from the PRIVATE GitHub repo.
 *
 * Flow:
 ...
 *     (octwbe-v), so other apps in the same monorepo don't interfere.
 *   - Surfaces the newest version on the WordPress "Updates" screen as a normal
 *     one-click update.
 ...
 *     NOT be forwarded).
 *
 * Mirrors the proven HGD_Updater pattern. No third-party library required.
 *
...
class OCTWBE_Updater {

	private string $basename;   // oct-bulk-editor/oct-bulk-editor.php
	private string $slug;       // oct-bulk-editor
	private string $version;    // installed version
	private string $repo;       // owner/repo
	private string $token;      // GitHub fine-grained token (Contents: read)
	private string $tag_prefix; // octwbe-v
	private string $cache_key;

	public function __construct( string $basename, string $version, string $repo, string $token, string $tag_prefix = 'octwbe-v' ) {
		...
		$this->cache_key  = 'octwbe_updater_' . md5( $this->repo . '|' . $this->tag_prefix );
	...
	/**
	 * Fetch and cache the latest matching release.
	 *
	...
	/** GitHub API asset URL for the first .zip on a release. */
	...
	public function check_for_update( $transient ) {
		...
		if ( $release && version_compare( $release['version'], $this->version, '>' ) ) {
			$transient->response[ $this->basename ] = (object) [
				...
				'package'     => 'octwbe-private:' . $release['zip'],
			];
		} else {
	...
	public function download_private_asset( $reply, $package, $upgrader = null ) {
		if ( ! is_string( $package ) || 0 !== strpos( $package, 'octwbe-private:' ) ) {
			return $reply;
		}
		$asset_url = substr( $package, strlen( 'octwbe-private:' ) );
		...
		if ( ( 301 === $code || 302 === $code || 307 === $code ) && $location ) {
			...
			$tmp  = wp_tempnam( 'octwbe-update.zip' );
			...
			if ( ! $tmp || ! $body || false === file_put_contents( $tmp, $body ) ) { // phpcs:ignore WordPress.WP.AlternativeFunctions
				return new WP_Error( 'octwbe_download_failed', __( 'Could not save the update package.', 'oct-bulk-editor' ) );
			}
		} else {
			return new WP_Error( 'octwbe_download_failed', sprintf(
				/* translators: %d: HTTP status code */
				__( 'GitHub returned status %d when downloading the update.', 'oct-bulk-editor' ),
				$code
			) );
		}
	...
	public function diagnose(): array {
		if ( '' === trim( $this->token ) ) {
			return [ 'ok' => false, 'message' => __( 'No GitHub token saved. Paste a fine-grained token with Contents: read on this repo.', 'oct-bulk-editor' ) ];
		}
		...
		if ( is_wp_error( $response ) ) {
			return [ 'ok' => false, 'message' => sprintf(
				/* translators: %s error */ __( 'Could not reach GitHub: %s', 'oct-bulk-editor' ),
		...
		if ( 200 !== $code ) {
			...
			$hint    = '';
			if ( 401 === $code ) {
				$hint = __( 'The token is invalid or expired.', 'oct-bulk-editor' );
			} elseif ( 403 === $code ) {
				$hint = __( 'Access forbidden — the org may require approval for fine-grained tokens, or the token lacks Contents: read.', 'oct-bulk-editor' );
			} elseif ( 404 === $code ) {
				$hint = __( 'Repo not found for this token — check the repository name and that the token is scoped to it (Resource owner = octobercomms).', 'oct-bulk-editor' );
			}
			return [ 'ok' => false, 'message' => sprintf(
				/* translators: 1: HTTP code, 2: github message, 3: hint */ __( 'GitHub returned HTTP %1$d (%2$s). %3$s', 'oct-bulk-editor' ),
				$code,
				$api_msg ? $api_msg : '—',
				$hint
			) ];
		...
		if ( ! is_array( $body ) ) {
			return [ 'ok' => false, 'message' => __( 'GitHub returned an unexpected response.', 'oct-bulk-editor' ) ];
		}
		...
		if ( 0 === $matching ) {
			return [ 'ok' => false, 'message' => sprintf(
				/* translators: %s tag prefix */ __( 'Connected to GitHub, but found no releases tagged "%s…" yet. The release Action publishes one when a new version lands on main.', 'oct-bulk-editor' ),
				$this->tag_prefix
			) ];
		}
		...
		return [ 'ok' => true, 'message' => sprintf(
			/* translators: 1: count, 2: latest version, 3: installed version */ __( 'Connected. Found %1$d releases; latest is %2$s (you have %3$s). Go to Dashboard → Updates → Check again to pull it in.', 'oct-bulk-editor' ),
			$matching,
			$latest,
			$this->version
		) ];
	...
	private function request_args( string $accept ): array {
		return [
			...
			'headers' => [
				...
				'User-Agent'           => 'OctoberComms-Bulk-Editor-Updater',
			],
		];
	}
}
