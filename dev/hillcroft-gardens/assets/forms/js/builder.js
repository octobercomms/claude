...
(function ($) {
	...
	var ROOT = document.getElementById('hgd-form-builder');
	if (!ROOT) return;
	...
	function syncHiddenInput() {
		var input = document.getElementById('hgd_form_schema_json');
	...
	function render() {
		...
		var body = el('div', { class: 'hgd-form-b-body' });
	...
	function toolbar() {
		function tab(id, label) {
			return el('button', { type: 'button',
				class: 'hgd-form-b-tab' + (state.view === id ? ' is-active' : ''),
				onClick: function () { state.view = id; render(); }
			}, [label]);
		}
		return el('div', { class: 'hgd-form-b-toolbar' }, [
			tab('steps',         'Steps'),
			tab('theme',         'Theme'),
			tab('brevo',         'Brevo'),
			...
			tab('endings',       'Ending'),
			tab('json',          'JSON'),
			el('div', { class: 'hgd-form-b-spacer' }),
			el('span', { class: 'hgd-form-b-hint' }, ['Save the post to persist changes.'])
		]);
	}

	function sidebar() {
		if (state.view !== 'steps') return el('div', { class: 'hgd-form-b-side hgd-form-b-side-empty' });

		var host = el('div', { class: 'hgd-form-b-side' });
		host.appendChild(el('h3', null, ['Steps']));
		var list = el('ol', { class: 'hgd-form-b-step-list' });
		schema.steps.forEach(function (s, i) {
			var li = el('li', { class: 'hgd-form-b-step-item' + (state.selectedStep === i ? ' is-active' : ''),
				onClick: function () { state.selectedStep = i; state.selectedQuestion = null; render(); }
			}, [
				el('span', { class: 'hgd-form-b-step-num' }, [String(i + 1)]),
				el('span', { class: 'hgd-form-b-step-title' }, [s.title || '(untitled step)']),
				el('span', { class: 'hgd-form-b-step-actions' }, [
					...
					el('button', { type: 'button', title: 'Delete', class: 'hgd-form-b-del', onClick: function (e) {
						e.stopPropagation();
	...
	function mainArea() {
		var host = el('div', { class: 'hgd-form-b-main' });
		switch (state.view) {
	...
	function inspector() {
		if (state.view !== 'steps') return el('div', { class: 'hgd-form-b-insp hgd-form-b-insp-empty' });
		var host = el('div', { class: 'hgd-form-b-insp' });
	...
	function renderStepEditor(host) {
		if (state.selectedStep == null) {
			host.appendChild(el('div', { class: 'hgd-form-b-empty' }, ['Add a step to get started.']));
			return;
		}
		var step = schema.steps[state.selectedStep];
		...
		var qList = el('div', { class: 'hgd-form-b-questions' });
		step.questions.forEach(function (q, i) {
			var typeLabel = (types[q.type] && types[q.type].label) || q.type;
			var card = el('div', { class: 'hgd-form-b-q-card' + (state.selectedQuestion === i ? ' is-active' : ''),
				onClick: function () { state.selectedQuestion = i; render(); }
			}, [
				el('div', { class: 'hgd-form-b-q-card-row' }, [
					el('span', { class: 'hgd-form-b-q-type' }, [typeLabel]),
					el('span', { class: 'hgd-form-b-q-id' }, [q.id])
				]),
				el('div', { class: 'hgd-form-b-q-card-label' }, [stripHtml(q.label || '(no label)')]),
				el('div', { class: 'hgd-form-b-q-card-actions' }, [
					...
					el('button', { type: 'button', class: 'hgd-form-b-del', onClick: function (e) {
						e.stopPropagation();
		...
		var palette = el('div', { class: 'hgd-form-b-palette' });
		palette.appendChild(el('div', { class: 'hgd-form-b-palette-title' }, ['Add a question:']));
	...
	function renderStepInspector(host, step) {
		...
		host.appendChild(el('label', { class: 'hgd-form-b-field hgd-form-b-field-inline' }, [
		...
		host.appendChild(el('p', { class: 'hgd-form-b-hint' }, ['By default the step title is an internal label for the builder only.']));
		host.appendChild(el('label', { class: 'hgd-form-b-field hgd-form-b-field-inline' }, [
	...
	function renderQuestionInspector(host, step, q) {
		...
		if (['choice', 'multi_choice', 'image_cards', 'image_cards_multi', 'dropdown'].indexOf(q.type) > -1) {
			...
			var opts = el('div', { class: 'hgd-form-b-opts' });
			q.options.forEach(function (opt, i) {
				var row = el('div', { class: 'hgd-form-b-opt' }, [
					...
					(q.type === 'image_cards' || q.type === 'image_cards_multi')
						? el('div', { class: 'hgd-form-b-opt-image' }, [
						...
						: null,
					el('button', { type: 'button', class: 'hgd-form-b-del', onClick: function () { q.options.splice(i, 1); render(); } }, ['×'])
				]);
	...
	function logicEditor(title, rules, onChange) {
		var host = el('div', { class: 'hgd-form-b-logic' });
		host.appendChild(el('h4', null, [title]));
		var hint = el('p', { class: 'hgd-form-b-hint' }, ['Leave empty to always show.']);
		host.appendChild(hint);
		var list = el('div', { class: 'hgd-form-b-logic-list' });

		function refresh() {
			list.innerHTML = '';
			rules.forEach(function (r, i) {
				var row = el('div', { class: 'hgd-form-b-logic-row' }, [
					i > 0 ? (function () {
					...
					el('button', { type: 'button', class: 'hgd-form-b-del', onClick: function () { rules.splice(i, 1); refresh(); onChange(rules); } }, ['×'])
				]);
	...
	function renderTheme(host) {
		...
		function color(label, key) {
			return el('label', { class: 'hgd-form-b-field' }, [
				el('span', null, [label]),
	...
	function renderBrevo(host) {
		...
		var datalistId = 'hgd-form-brevo-attrs';
		...
		host.appendChild(el('p', { class: 'hgd-form-b-hint' }, [
		...
		host.appendChild(el('p', { class: 'hgd-form-b-hint', style: { background: '#fffbeb', border: '1px solid #fde68a', padding: '8px 10px', borderRadius: '4px' } }, [
			(function () {
	...
	function mapEditor(host, map, onChange, datalistId) {
		var list = el('div', { class: 'hgd-form-b-map' });
		function rebuild() {
			list.innerHTML = '';
			Object.keys(map).forEach(function (qid) {
				...
				var row = el('div', { class: 'hgd-form-b-map-row' }, [
					(function () {
					...
					input,
					el('button', { type: 'button', class: 'hgd-form-b-del', onClick: function () { delete map[qid]; rebuild(); onChange(); } }, ['×'])
				]);
				list.appendChild(row);
			});
		}
