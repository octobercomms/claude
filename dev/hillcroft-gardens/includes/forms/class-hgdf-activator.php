...
class HGDF_Activator {

	public static function activate() {
		global $wpdb;
		$charset_collate = $wpdb->get_charset_collate();
		$submissions     = $wpdb->prefix . 'hgd_form_submissions';
		$uploads         = $wpdb->prefix . 'hgd_form_uploads';
		$events          = $wpdb->prefix . 'hgd_form_events';
		$views           = $wpdb->prefix . 'hgd_form_views';
		...
		update_option( 'hgd_form_db_version', HGDF_DB_VERSION );

		if ( ! wp_next_scheduled( 'hgd_form_retry_events' ) ) {
			wp_schedule_event( time() + 60, 'hourly', 'hgd_form_retry_events' );
		}
	}

	public static function deactivate() {
		$ts = wp_next_scheduled( 'hgd_form_retry_events' );
		if ( $ts ) {
			wp_unschedule_event( $ts, 'hgd_form_retry_events' );
		}
	}
