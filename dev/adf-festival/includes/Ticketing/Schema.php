...
namespace ADF\Ticketing;

defined('ABSPATH') || exit;

/**
 * Ticketing schema — relational tables, mirroring the proven Event Tickets
 * model (orders → tickets, check-ins, promo codes) but with an `adf_` prefix.
 *
 * This replaces the earlier flat `adf_ticket` CPT so an event can sell multiple
 * ticket types, group ("admits N") tickets, and so check-ins/promos/capacity
 * work relationally and cheaply.
 */
final class Schema {

    public static function orders(): string   { global $wpdb; return $wpdb->prefix . 'adf_orders'; }
    public static function tickets(): string  { global $wpdb; return $wpdb->prefix . 'adf_tickets'; }
    public static function checkins(): string { global $wpdb; return $wpdb->prefix . 'adf_checkins'; }
    public static function promos(): string   { global $wpdb; return $wpdb->prefix . 'adf_promo_codes'; }
