...
function Overview({ summary, findings }) {
  ...
  return (
    <div className="grid grid-2" style={{ marginTop: 'var(--s4)', alignItems: 'stretch' }}>
      <div className="card">
        <div className="caption caption-muted" style={{ marginBottom: 'var(--s2)' }}>Funnel health</div>
        <p className="body" style={{ margin: 0 }}>{summary || '—'}</p>
      </div>
      <div className="card" style={{ display: 'flex', alignItems: 'center', gap: 'var(--s5)', flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: 180 }}>
          ...
          <div style={{ marginTop: 'var(--s3)', paddingTop: 'var(--s3)', borderTop: '1px solid var(--card-border)' }}>
...
function FindingCard({ f, open, onToggleExpand, onToggleDone, showSite }) {
  ...
  return (
    <div className="card" style={{ padding: 0, overflow: 'hidden', borderLeft: `4px solid ${p.color}`, opacity: f.done ? 0.6 : 1, transition: 'opacity .15s' }}>
      <div onClick={onToggleExpand} style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--s3)', padding: 'var(--s4) var(--s4)', cursor: 'pointer' }}>
        ...
        <div style={{ minWidth: 0, flex: 1 }}>
          ...
          {open && f.fix && (
            <div className="body-sm" style={{ marginTop: 'var(--s3)', padding: 'var(--s3) var(--s3)', background: 'var(--surface-sunken)', borderRadius: 'var(--r-sm)' }}>
              <strong style={{ color: p.color }}>Fix:</strong> {f.fix}
            </div>
          )}
        </div>
        <span style={{ flex: '0 0 auto', color: 'var(--text-subtle)', fontSize: 'var(--fs-caption)', marginTop: 'var(--s1)', transform: open ? 'rotate(180deg)' : 'none', transition: 'transform .15s' }}>▾</span>
      </div>
    </div>
  );
}
...
export default function ClarityCroPanel({ clientId }) {
  ...
  return (
    <div>
      ...
      {sites.length > 1 && (
        <div className="row" style={{ alignItems: 'center', gap: 'var(--s2)', marginBottom: 'var(--s4)' }}>
      ...
      {selected !== 'all' && (
        <div className="card" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 'var(--s3)', flexWrap: 'wrap' }}>
          <div>
      ...
      {viewFindings.length === 0 ? (
        <p className="body-sm text-subtle" style={{ marginTop: 'var(--s4)' }}>
          {selected === 'all'
