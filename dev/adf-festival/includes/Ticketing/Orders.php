...
final class Orders {
    ...
    public static function create(array $data, string $payment_id = '', string $method = 'stripe', string $source = 'public') {
        ...
        if ($source === 'public') {
            ...
            if ($avail['state'] !== 'available') {
                return new \WP_Error('adf_unavailable', __('Those tickets are no longer available.', 'adf-festival'));
            }
            $cap = $type['capacity'] ?? null;
            if ($cap !== null) {
                $would = TicketTypes::sold_count($event_id, $type['key']) + ($qty * (int) $type['qty_per_purchase']);
                if ($would > (int) $cap) {
                    return new \WP_Error('adf_capacity', __('Not enough tickets remain for that quantity.', 'adf-festival'));
                }
            }
        }
        ...
        if (! $order_id) {
            return new \WP_Error('adf_order_failed', __('Could not create the order.', 'adf-festival'));
        }
    ...
    public static function create_manual(int $event_id, string $type_key, int $qty, string $name, string $email, bool $paid) {
        ...
        if (! $type) {
            return new \WP_Error('adf_bad_type', __('Unknown ticket type.', 'adf-festival'));
        }
    ...
    public static function send_confirmation(int $order_id): void {
        ...
        if (isset($lists['adf_event_attendees'])) {
            BrevoConnector::upsert_contact($order->email, [], [(int) $lists['adf_event_attendees']]);
        }
